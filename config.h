...
static const unsigned int systrayspadding = 4; /* 托盘和状态栏的间隙 */
...
static const char *tags[] = {
    ...
    "",  // tag:7  key:4  desc:terminal4
    "",  // tag:8  key:5  desc:terminal5
    "󰑈",  // tag:9  key:b  desc:bilibili
    "",  // tag:10 key:y  desc:idea
    "",  // tag:11 key:w  desc:warp
};
...
static const Rule rules[] = {
    ...
    {"firefox-beta", NULL, NULL, 1 << 4, 0, 0, 0, -1, 0},  // firefox     tag -> 
...
static Key keys[] = {
    ...
    {MODKEY,
     XK_r,
     incnmaster,
     {.i = +1}}, /* super r            |  改变主工作区窗口数量 (1 2中切换) */

    {MODKEY,
     XK_n,
     focusmon,
     {.i = +1}}, /* super n            |  光标移动到另一个显示器 */
    {MODKEY | ShiftMask,
     XK_n,
     tagmon,
     {.i = +1}}, /* super shift n      |  将聚焦窗口移动到另一个显示器 */
    ...
    {MODKEY, XK_e, spawn, 
     SHCMD("killall pcmanfm || pcmanfm")},  /* super e | 打开或关闭pcmanfm  */
    {MODKEY, XK_d, spawn,
     SHCMD("rofi -show drun")}, /* super d          | rofi: 执行run          */
    {MODKEY, XK_p, spawn,
     SHCMD("$DWM/DEF/rofi.sh")}, /* super p          | rofi: 执行自定义脚本   */
    {MODKEY, XK_x, spawn,
     SHCMD("$DWM/DEF/blurlock.sh")}, /* super x          | 锁定屏幕 */
    ...
    {MODKEY | ShiftMask, XK_a, spawn,
     SHCMD("flameshot gui -c -p ~/Pictures/screenshots")}, /* super shift a    |*/
    {MODKEY | ShiftMask, XK_q, spawn,
     SHCMD("kill -9 $(xprop | grep _NET_WM_PID | awk '{print $3}')")},/* super shift q    | 选中某个窗口并强制kill */
    ...
    TAGKEYS(XK_3, 2, 0)
    TAGKEYS(XK_9, 3, "obs")
    TAGKEYS(XK_c, 4, "firefox-beta")
    TAGKEYS(XK_m, 5, "~/scripts/music_player.sh")
    TAGKEYS(XK_0, 6, "linuxqq")
    TAGKEYS(XK_4, 7, 0)
    TAGKEYS(XK_5, 8, 0)
    TAGKEYS(XK_b, 9, "~/Applications/bilibili-1.13.2_6d2be29c0f5e87c4afce37fe82837f7f.AppImage")
    TAGKEYS(XK_y, 10, "st")
    TAGKEYS(XK_w, 11, "warp-terminal")
